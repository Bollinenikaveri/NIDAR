...
const Header = ({ onMaxFlightTimeChange }) => {
  ...
  const [showSettings, setShowSettings] = useState(false);
  const [activeTab, setActiveTab] = useState('ros');
  const [isDeliveryListOpen, setIsDeliveryListOpen] = useState(true); // For the accordion
  const [unsavedChanges, setUnsavedChanges] = useState({
    ros: false,
    video: false,
    time: false,
  ...
  const [rosSettings, setRosSettings] = useState(defaultRosSettings);
  ...
  // Load saved settings from zustand store
  useEffect(() => {
    const storeState = useStore.getState();

    // *** NEW DEFAULT-MERGING LOGIC FOR ROS SETTINGS ***
    const loadedRosSettings = storeState.rosSettings
      ? { ...defaultRosSettings, ...storeState.rosSettings }
      : defaultRosSettings;

    // Clean up old 'drones' property if it exists from old storage
    if (loadedRosSettings.drones) {
      delete loadedRosSettings.drones;
    }
    
    // Ensure deliveryDrones is always an array
    if (!Array.isArray(loadedRosSettings.deliveryDrones)) {
      loadedRosSettings.deliveryDrones = [];
    }

    setRosSettings(loadedRosSettings);
    // *** END NEW LOGIC ***
    ...
    // We only want this to run once on mount, and onMaxFlightTimeChange is a dep
    
  }, [onMaxFlightTimeChange]);

  // Helper: persist current store-backed settings
  const persistSettingsToLocal = () => {
    try {
      const s = useStore.getState();
      const payload = {
  ...
  // *** NEW HANDLERS FOR DELIVERY DRONES ***

  const handleAddDeliveryDrone = () => {
    const newDrone = {
      id: Date.now(), // Simple unique ID
      name: `Drone ${rosSettings.deliveryDrones.length + 1}`,
      address: '192.168.1.100', // Default address
      connected: false
    };
    const newRosSettings = {
      ...rosSettings,
      deliveryDrones: [...rosSettings.deliveryDrones, newDrone]
    };
    setRosSettings(newRosSettings);
    setUnsavedChanges({ ...unsavedChanges, ros: true });
    useStore.getState().setRosSettings(newRosSettings);
  ...
  const handleRemoveDeliveryDrone = (id) => {
    const newRosSettings = {
      ...rosSettings,
      deliveryDrones: rosSettings.deliveryDrones.filter(d => d.id !== id)
    };
    setRosSettings(newRosSettings);
    setUnsavedChanges({ ...unsavedChanges, ros: true });
    useStore.getState().setRosSettings(newRosSettings);
    persistSettingsToLocal();
    toast.error("Drone removed");
  };

  const handleDeliveryDroneChange = (id, field, value) => {
    const newRosSettings = {
      ...rosSettings,
      deliveryDrones: rosSettings.deliveryDrones.map(d =>
        d.id === id ? { ...d, [field]: value } : d
      )
    };
    setRosSettings(newRosSettings);
    setUnsavedChanges({ ...unsavedChanges, ros: true });
    // This persists on every keystroke, which matches your original pattern
    useStore.getState().setRosSettings(newRosSettings);
    persistSettingsToLocal();
  };

  const handleDeliveryDroneConnect = (id) => {
    let droneName = '';
    ...
    toast.success(`${droneName} ${isConnecting ? 'Connected' : 'Disconnected'}!`);
  };
  ...
  const renderSettingsContent = () => {
    switch (activeTab) {
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="bg-gray-800/50 rounded p-4">
              ...
              <Button onClick={handleRosConnect} className={`w-full ${rosSettings.connected ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700'}`}>
                <Wifi className="w-4 h-4 mr-2" />
                {rosSettings.connected ? 'Disconnect' : 'Connect'} to ROS2
              </Button>
              {rosSettings.connected && (
            ...
            {/* --- New Delivery Drones Accordion --- */}
            <div className="bg-gray-800/50 rounded">
              {/* Accordion Header */}
              <button
                onClick={() => setIsDeliveryListOpen(!isDeliveryListOpen)}
                className="w-full flex justify-between items-center p-4 hover:bg-gray-700/50 rounded-t"
              >
                <h4 className="text-gray-300 text-sm font-semibold">Delivery Drones</h4>
                <div className="flex items-center space-x-2">
                  <span className="text-xs text-gray-400">
                    {rosSettings.deliveryDrones.length} drones
                  </span>
                  {isDeliveryListOpen ? (
              ...
              {isDeliveryListOpen && (
                <div className="p-4 border-t border-gray-700/50 space-y-4">
                  {rosSettings.deliveryDrones.length === 0 && (
                    <p className="text-center text-gray-500 text-sm py-4">No delivery drones added.</p>
                  )}

                  {/* List of Drones */}
                  {rosSettings.deliveryDrones.map(drone => (
                    <div key={drone.id} className="bg-gray-900/50 p-3 rounded-lg space-y-3">
                      <div className="flex justify-between items-center">
                        <input
                          type="text"
                          value={drone.name}
                          onChange={e => handleDeliveryDroneChange(drone.id, 'name', e.target.value)}
                          className="bg-transparent text-white text-sm font-semibold w-2/3 focus:outline-none focus:ring-1 focus:ring-blue-500 rounded px-1"
                        />
                        <button
                          onClick={() => handleRemoveDeliveryDrone(drone.id)}
                      ...
                      <div>
                        <label className="text-xs text-gray-400">Address (IP/Host)</label>
                        <input
                          type="text"
                          value={drone.address}
                          onChange={e => handleDeliveryDroneChange(drone.id, 'address', e.target.value)}
                          className="w-full bg-gray-700 text-white rounded px-3 py-1 text-sm mt-1"
                          placeholder="e.g., 192.168.1.101"
                        />
                      </div>

                      <button
                        onClick={() => handleDeliveryDroneConnect(drone.id)}
                        className={`w-full px-3 py-1.5 rounded text-xs font-medium ${drone.connected ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700'}`}
                      >
                        {drone.connected ? 'Disconnect' : 'Connect'}
                      </button>
                      {drone.connected && (
                  ...
                  {/* Add Drone Button */}
                  <Button
                    onClick={handleAddDeliveryDrone}
                    variant="outline"
                    className="w-full border-dashed border-gray-600 text-gray-400 hover:bg-gray-700/50 hover:text-white"
                  >
      ...
      case 'time':
        ...
        const { maxFlightTime } = timeSettings;
        return (
          <div className="space-y-6">
            <h3 className="text-lg font-semibold text-blue-400">Flight Time Management</h3>
            <div className="bg-gray-800/50 rounded p-4">
  ...
  return (
    <div className="relative">
      ...
      {/* Sidebar */}
      <div
        className={`fixed top-0 left-0 h-full w-96 bg-gray-900 border-r border-gray-700 z-50 transition-transform duration-300 ${showSettings ? 'translate-x-0' : '-translate-x-full'
          }`}
      >
        <div className="flex h-full">
          <div className="w-24 bg-gray-800/50 border-r p-4">
            ...
            <nav className="space-y-2">
              {sidebarItems.map(item => (
                <button
                  key={item.id}
                  onClick={() => setActiveTab(item.id)}
                  className={`w-full flex flex-col items-center p-2 rounded-lg transition ${activeTab === item.id
                      ? 'bg-blue-600/20 text-blue-400 border border-blue-600/30'
                      : 'text-gray-300 hover:bg-gray-700/50 hover:text-white'
                    }`}
                  title={item.label}
